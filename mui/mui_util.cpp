...
COLOR_HSL RGBtoHSL( COLORREF rgb )
{
	double cHsl[3];
	RGBToHSV( GetRValue( rgb ) / 255.0f, GetGValue( rgb ) / 255.0f, GetBValue( rgb ) / 255.0f, cHsl[0], cHsl[1], cHsl[2] );
	COLOR_HSL hsl;
	hsl.hue = cHsl[0];
	hsl.saturation = cHsl[1];
	hsl.luminance = cHsl[2];
	return hsl;
}
COLORREF HSLtoRGB( COLOR_HSL hsl )
{
	double cRgb[3];
	HSVToRGB( hsl.hue, hsl.saturation, hsl.luminance, cRgb[0], cRgb[1], cRgb[2] );
	return RGB( cRgb[0] * 255, cRgb[1] * 255, cRgb[2] * 255 );
}